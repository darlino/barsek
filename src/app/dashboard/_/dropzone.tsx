    ...
    return (
      <div className="space-y-6 w-full">
        {actions.map((action: Action, i: any) => (
          <div
            key={i}
            className="w-full shadow-md py-4 space-y-2 lg:py-0 relative cursor-pointer rounded-lg border h-fit lg:h-20 px-4 lg:px-4 flex flex-wrap lg:flex-nowrap items-center justify-between"
          >
            {!is_loaded && (
            ...
            {action.is_error ? (
              <Badge variant="destructive" className="flex gap-2">
                <span>Erreur lors de la conversion</span>
                <BiError />
              </Badge>
            ) : action.is_converted ? (
